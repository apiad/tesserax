...
def hex(*args, **kwargs):
    if isinstance(args[0], str):
        ...
        return Color(int(r, 16), int(g, 16), int(b, 16), alpha)
    else:
        color: Color = args[0]
...
class Colors:
    ...
    def get(cls, name: str) -> Color:
        try:
            return cls.__colors[name]
        except KeyError:
            raise ValueError(f"No color named {repr(name)}.")

    @classmethod
    def get_name(cls, color: Color) -> str:
        try:
            return Colors.__color_names[color]
        except KeyError:
            raise ValueError(f"Color {repr(color)} not registered.")
