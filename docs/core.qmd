...
with Canvas() as canvas:
    Rect(150, 40, fill=Colors.LightBlue)
    Text("Hello World", size=24, font="serif")

canvas.align().fit(10).display()
```
...
with Canvas() as canvas:
    ...
    Circle(30, fill="salmon").translated(100, 0)
...
```{python}
from tesserax import Canvas, Rect, Circle, Group, deg
from tesserax.layout import RowLayout

with Canvas() as canvas:
    # Create and rotate a layout
    with RowLayout(gap=20).rotated(deg(45)):
        Rect(100, 50, fill=Colors.LightBlue)
        Circle(30, fill=Colors.Salmon)

    # Add another shape to the scene
    Rect(40, 40, fill=Colors.LightGreen).translated(100, -50)
...
```{python}
from tesserax import Canvas, Rect, Circle, Arrow, deg
from tesserax.layout import RowLayout

with Canvas() as canvas:
    # 1. Create a Row and rotate it
    with RowLayout(gap=40) as row:
        r1 = Rect(80, 40, fill=Colors.AliceBlue)
        c1 = Circle(30, fill=Colors.MistyRose)

    row.rotated(deg(45)).translated(50, 50)

    # 2. Create a target rectangle outside the layout
    target = Rect(60, 60, fill=Colors.Honeydew).translated(250, 0)
...
with Canvas() as canvas:
    ...
    p = Path()
    p.jump_to(0, 0).line_to(50, 50).line_to(0, 50).close()
    p.translated(20, 20)

    # 2. A curved path using Bezier curves
    curve = Path()
    curve.jump_to(100, 20)
...
with Canvas() as canvas:
    ...
    Polyline(points, smoothness=0.3, stroke=Colors.Blue).translated(0, 60)

    # 3. Fully Smooth (100% smoothing)
    # This creates a flowing wave-like appearance, and closed.
    Polyline(points, smoothness=1.0, stroke=Colors.Red, closed=True).translated(0, 120)
...
```{python}
from tesserax import Canvas, Circle, Line, Polyline
from tesserax.layout import RowLayout
from tesserax.path import Grid
from tesserax.color import Colors

with Canvas() as canvas:
    with RowLayout(gap=30) as row:
        c1 = Circle(20, fill=Colors.LightBlue)
        c2 = Circle(30, fill=Colors.LightPink)
        c3 = Circle(40, fill=Colors.LightPink)

    g = Grid(row)

    Polyline(
        g.trace(c1.anchor("left").dx(-5), c3.anchor("right").dx(5)),
        smoothness=1,
        marker_end="arrow",
    )

    Polyline(
        g.trace(c2.anchor("bottom").dy(5), c3.anchor("bottom").dy(5)),
        smoothness=1,
        marker_end="arrow",
    )
